        ...
        {experts.map((expert) => (
          <article key={expert.id} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-[0_1px_3px_rgba(16,32,54,0.04)]">
            <div className="flex items-center gap-4">
              <span className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-[#1A7A52] to-[#1A7A52] text-[16px] font-bold text-white">
                {expert.avatar ? (
              ...
              <div className="min-w-0">
                <h3 className="flex items-center gap-1.5 text-[16px] font-bold leading-tight text-[#191919]">
                  ...
                  <BadgeCheck className="h-4 w-4 shrink-0 text-[#1A7A52]" strokeWidth={2} />
                </h3>
                <p className="mt-0.5 text-[13px] font-semibold text-[#1A7A52]">{expert.role}</p>
              </div>
            </div>
            ...
            {expert.specialties && expert.specialties.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-1.5">
                {expert.specialties.map((s) => (
                  <span key={s} className="rounded-full bg-[#1A7A52]/[0.06] px-2.5 py-1 text-[11px] font-semibold text-[#1A7A52]">
                    {s}
                  </span>
                ))}
              </div>
