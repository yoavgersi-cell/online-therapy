    ...
    <article className="relative rounded-md border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md overflow-hidden">
      ...
      <div className="hidden sm:flex sm:flex-row sm:min-h-[196px]">
        ...
        <div className="flex-1 px-6 py-5">
          ...
          <a
            ...
            className="text-[12px] font-semibold text-[#1A7A52] hover:underline"
          >
            Read Review
          </a>
          ...
          <ul className="mt-1 space-y-0.5">
            {product.highlights.slice(0, 3).map((highlight) => (
              <li key={highlight} className="flex items-center gap-2 text-[12px] leading-[1.35] text-gray-800">
                <Check className="h-3.5 w-3.5 shrink-0 text-[#1A7A52]" strokeWidth={2} />
                {highlight}
              </li>
            ))}
            {product.highlights[3] && (
              <li className="mt-1.5 inline-flex items-center gap-1.5 whitespace-nowrap rounded-md bg-[#FEF3D0] px-2.5 py-1 text-[11.5px] font-semibold leading-[1.3] text-[#8A6413] ring-1 ring-inset ring-[#F3DFA6]">
        ...
        <div className="relative flex flex-col items-center justify-center gap-4 px-6 py-5 sm:w-[200px] sm:shrink-0">
          ...
          <div className="relative w-full">
            ...
            <ProviderCta
              ...
              className="flex h-[42px] w-full items-center justify-center gap-2 rounded-lg bg-[#1A7A52] text-[15px] font-bold text-white transition-colors hover:bg-[#14634A]"
            >
              Visit Site
              {arrowSvg}
            </ProviderCta>
      ...
      <div className="flex flex-col sm:hidden p-5 pt-12">
        ...
        <ul className="mt-1.5 space-y-1">
          {product.highlights.slice(0, 3).map((highlight) => (
            <li key={highlight} className="flex items-start gap-2 text-[14px] leading-[1.4] text-gray-800">
              <Check className="mt-0.5 h-4 w-4 shrink-0 text-[#1A7A52]" strokeWidth={2} />
              {highlight}
            </li>
          ))}
          {product.highlights[3] && (
            <li className="mt-2 inline-flex items-center gap-1.5 self-start whitespace-nowrap rounded-md bg-[#FEF3D0] px-2.5 py-1.5 text-[13px] font-semibold leading-[1.3] text-[#8A6413] ring-1 ring-inset ring-[#F3DFA6]">
        ...
        <ProviderCta
          ...
          pageType={pageType}
          sourceFlow={sourceFlow}
          className="mt-5 flex h-[48px] w-full items-center justify-center gap-2 rounded-lg bg-[#1A7A52] text-[16px] font-bold text-white transition-colors hover:bg-[#14634A]"
        >
          Visit Site
          {arrowSvg}
        </ProviderCta>
