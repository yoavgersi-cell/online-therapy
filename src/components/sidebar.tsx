        ...
        return (
          <SidebarCard key="socialProof">
            <div className="flex items-center gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#1A7A52]/8">
                <Users className="h-6 w-6 text-[#1A7A52]" strokeWidth={1.5} />
              </div>
              <div>
            ...
            <div className="space-y-5">
              {topProviders.map((p) => (
                <Link key={p.id} href={`${linkPrefix}/reviews/${p.id}`} className="flex items-center gap-4 group">
                  ...
                  <div>
                    <p className="text-[15px] font-bold text-[#191919] group-hover:text-[#1A7A52] transition-colors">{p.name}</p>
                    <p className="text-[13px] text-gray-500">Read Review</p>
                  </div>
                </Link>
              ))}
            </div>
            <Link href={`${linkPrefix}/reviews`} className="mt-8 block text-[14px] font-semibold text-[#1A7A52] hover:underline">
              Read All Reviews
            </Link>
          </SidebarCard>
        );
        ...
        return (
          <SidebarCard key="rankingMethodology">
            ...
            <div className="space-y-3">
              {[
                { label: "Clinical credibility", pct: 92 },
