...
// "weight-loss"). This single-vertical build keeps the maps empty and renders
// battles from the ED config (src/lib/seeds/ed.ts). Seed a battle here only with
...
// feature (gated to ctx.vertical === "weight-loss"). Empty on this build.
...
export async function battleMetadata(slug: string, ctx: SiteContext): Promise<Metadata> {
  ...
  const metaTitle = override?.title
    ...
    : `${titleCaseMatchup(baseLabel)} (2026): ${battleCategory ? `${battleCategory} ` : ""}Cost, Plans & Sessions Compared`;
  ...
  return (
    <div className="mb-6 max-w-[820px]">
      ...
      <details className="group mt-1.5">
        <summary className="inline-flex cursor-pointer list-none items-center gap-1 text-[14px] font-semibold text-[#1A7A52] hover:underline [&::-webkit-details-marker]:hidden">
          {label}
...
export async function BattlePageView({ slug, ctx }: { slug: string; ctx: SiteContext }) {
  ...
  const quickRows = [
    ...
    { label: "Clinicians", v1: "Licensed in your state", v2: "Licensed in your state" },
  ...
  const catToQuestion = (name: string) => {
    ...
    if (n.includes("focus") || n.includes("simplic")) return `Which is more focused on therapy, ${p1.name} or ${p2.name}?`;
    if (n.includes("range") || n.includes("beyond")) return `Does ${p2.name} offer more than therapy compared to ${p1.name}?`;
    ...
    <>
      ...
      <div className="min-h-screen bg-[#FAFAFA]">
        ...
        <section className="relative overflow-hidden border-b border-gray-200 bg-white">
          ...
          <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#1A7A52] via-[#4FB286] to-[#1A7A52]" />
          ...
          <div className="mx-auto max-w-[1100px] px-4 pb-6 pt-5 sm:px-6 sm:pb-14 sm:pt-12">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-1.5 gap-y-1 text-[11.5px] leading-[1.5] text-gray-500 sm:mt-4 sm:text-[12.5px]">
              {config.experts && config.experts.length > 0 && (
                <span>
                  By{" "}
                  <Link href={hubLink(ctx, "/about")} className="font-medium text-gray-600 underline-offset-2 hover:text-[#1A7A52] hover:underline">
        ...
        <div className="mx-auto max-w-[1100px] px-4 pt-10 pb-28 sm:px-6 sm:pb-10">
          ...
          {verdictWinner && verdictRunnerUp && shortAnswerReason && (
            <div className="mb-8 max-w-[820px] overflow-hidden rounded-2xl border border-[#1A7A52]/20 bg-white shadow-sm">
              <div className="flex items-center justify-between gap-3 border-b border-gray-100 bg-[#F3F9FD] px-5 py-3 sm:px-6">
                <p className="text-[12px] font-bold uppercase tracking-[0.08em] text-[#1A7A52]">
                  The verdict
                </p>
                {showShortAnswerScores && (
                  <span className="flex items-center gap-1.5 whitespace-nowrap rounded-full bg-white px-2.5 py-1 text-[11.5px] font-bold text-[#1A7A52] shadow-sm ring-1 ring-[#1A7A52]/15">
          ...
          {ctx.vertical === "weight-loss" &&
            [p1, p2].some((p) => REDDIT_COMMUNITY_FEEDBACK[p.id]?.themes) && (
              <div className="mb-14 max-w-[820px] rounded-2xl border border-gray-200 bg-gray-50/60 p-5 sm:p-6">
                <p className="mb-3 text-[12px] font-bold uppercase tracking-[0.07em] text-[#1A7A52]">
                  What we found
                </p>
          ...
          {quickRows.length > 0 && (
            <div className="mb-12 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
              ...
              <p className="border-t border-gray-100 px-4 py-2.5 text-[11.5px] text-gray-400">
                ...
                <Link href={hubLink(ctx, "/how-we-rank")} className="font-medium text-gray-500 underline underline-offset-2 hover:text-[#1A7A52]">
          ...
          <div className="relative mb-12 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="absolute left-1/2 top-1/2 z-10 hidden -translate-x-1/2 -translate-y-1/2 sm:flex">
              <div className="flex h-11 w-11 items-center justify-center rounded-full border-2 border-[#1A7A52]/20 bg-white text-[13px] font-extrabold text-[#1A7A52] shadow-sm">
                VS
              </div>
            </div>
            ...
            {[p1, p2].map((provider, idx) => {
              ...
              <Fragment key={provider.id}>
                ...
                {idx === 1 && (
                  <div className="flex items-center justify-center sm:hidden -my-1">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full border-2 border-[#1A7A52]/20 bg-white text-[12px] font-extrabold text-[#1A7A52]">
                      VS
                    </div>
                  </div>
                )}
              <div className="relative flex h-full flex-col rounded-2xl border border-gray-200 bg-white px-6 pb-6 pt-7 shadow-sm">
                ...
                <ProviderCta
                  ...
                  className="mt-auto flex h-[44px] w-full items-center justify-center gap-1.5 rounded-xl bg-[#1A7A52] text-[14px] font-bold text-white transition-colors hover:bg-[#14634A]"
                >
                  Visit {provider.name}
          ...
          <div className="mb-14">
            <div className="mb-8 max-w-[760px]">
              <p className="mb-2.5 text-[12px] font-bold uppercase tracking-[0.07em] text-[#1A7A52]">
                The full breakdown
              </p>
            ...
            <div className="space-y-10">
              {deepDives.map(({ provider, review, bestForFallback }) => {
                ...
                return (
                  ...
                  >
                    {/* Accent bar */}
                    <div className="h-1 bg-gradient-to-r from-[#1A7A52] via-[#4FB286] to-[#1A7A52]" />
                    ...
                    <div className="p-6 sm:p-8">
                      ...
                      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-6">
                        ...
                        <ProviderCta
                          ...
                          pageType="battle"
                          sourceFlow="battle_page"
                          className="inline-flex h-[42px] items-center justify-center gap-1.5 rounded-xl bg-[#1A7A52] px-5 text-[14px] font-bold text-white transition-colors hover:bg-[#14634A]"
                        >
                          Visit {provider.name}
                          <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.5} />
                        </ProviderCta>
                      ...
                      <div className="mb-8 grid gap-6 sm:grid-cols-2">
                        ...
                        {bestFor.length > 0 && (
                          <div>
                            <h4 className="mb-3 flex items-center gap-2 text-[15px] font-bold text-[#191919]">
                              <Sparkles className="h-4 w-4 text-[#1A7A52]" strokeWidth={2} />
                              Who {provider.name} is best for
                            </h4>
                            <ul className="space-y-2">
                              {bestFor.map((f, i) => (
                                <li key={i} className="flex items-start gap-2 text-[14px] leading-[1.6] text-gray-600">
                                  <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#1A7A52]" strokeWidth={2} />
                                  {f}
                                </li>
                              ))}
                            </ul>
                            ...
                            {plans.map((plan, i) => (
                              <div key={i} className="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
                                ...
                                <div className="flex items-baseline gap-1.5">
                                  <span className="text-[22px] font-extrabold text-[#1A7A52]">{plan.price}</span>
                                  {plan.regularPrice && (
                      ...
                      <ProviderCta
                        ...
                        pageType="battle"
                        sourceFlow="battle_page"
                        className="flex h-[48px] w-full items-center justify-center gap-2 rounded-xl bg-[#1A7A52] text-[15px] font-bold text-white transition-colors hover:bg-[#14634A]"
                      >
              ...
              <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                {[
                  ...
                  >
                    ...
                    <span className="shrink-0 text-[13.5px] font-bold text-[#1A7A52] sm:text-[14px]">{pick}</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {runnerUpPts.slice(0, 3).map((pt, i) => (
                      <li key={i} className="flex items-start gap-2 text-[14px] leading-[1.65] text-gray-600">
                        <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#1A7A52]" strokeWidth={2.5} />
                        <span><BoldKeyFacts text={pt} /></span>
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mb-6 grid grid-cols-2 gap-4">
            {[
              { provider: p1, color: "from-gray-50 to-gray-50/50", hoverColor: "hover:border-[#1A7A52]/30", textColor: "text-[#191919]" },
              { provider: p2, color: "from-gray-50 to-gray-50/50", hoverColor: "hover:border-[#1A7A52]/30", textColor: "text-[#191919]" },
                ...
                <div className="mb-3 grid gap-3">
                  {relatedTrios.map((t) => (
                    <Link
                      ...
                      className="group flex items-center gap-3 rounded-xl border border-[#1A7A52]/20 bg-[#1A7A52]/[0.03] px-4 py-3.5 transition-colors hover:border-[#1A7A52]/40 hover:bg-[#1A7A52]/[0.06]"
                    >
                      ...
                      <span className="ml-auto inline-flex shrink-0 items-center gap-1 text-[13px] font-semibold text-[#1A7A52] group-hover:underline">
                        Compare all three
                        <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.5} />
                      </span>
                    </Link>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {relatedBattles.map(({ slug: relSlug, bp1, bp2 }) => (
                  <Link
                    ...
                    className="group flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3.5 transition-colors hover:border-[#1A7A52]/30 hover:bg-[#1A7A52]/[0.02]"
                  >
                    ...
                    <span className="ml-auto inline-flex items-center gap-1 text-[13px] font-semibold text-[#1A7A52] group-hover:underline">
                      Compare
                      <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.5} />
                    </span>
                  </Link>
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 py-4 text-[13px]">
            <Link href={hubLink(ctx, `/reviews/${p1.id}`)} className="font-semibold text-[#1A7A52] hover:underline">
              {p1.name} Review
            </Link>
            <span className="text-gray-300">|</span>
            <Link href={hubLink(ctx, `/reviews/${p2.id}`)} className="font-semibold text-[#1A7A52] hover:underline">
              {p2.name} Review
            </Link>
            <span className="text-gray-300">|</span>
            <Link href={hubLink(ctx, "/")} className="font-semibold text-[#1A7A52] hover:underline">
