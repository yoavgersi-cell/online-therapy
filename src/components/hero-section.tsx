...
export function HeroSection({
  ...
  return (
    <section className="relative w-full py-5 sm:py-0 sm:h-[260px] lg:h-[290px] overflow-hidden bg-[#E8F5EE]">
      ...
      >
        <div className="max-w-[640px] lg:max-w-[900px]" style={{ maxWidth: maxTextWidth !== "640px" ? maxTextWidth : undefined }}>
          ...
          {h2 && (
            <h2 className="mt-2 hidden sm:block text-[24px] lg:text-[26px] leading-[1.25] font-semibold text-[#1A7A52]">
              {h2}
            </h2>
          )}
          ...
          <div className="mt-4 sm:mt-3 flex items-center gap-3 sm:gap-5">
            {[
              { label: "Licensed Therapists", icon: Stethoscope },
              { label: "Top-Rated Platforms", icon: Star },
              { label: "Start Within Days", icon: Zap },
            ].map(({ label, icon: Icon }) => (
              <span key={label} className="flex items-center gap-1 text-[11px] sm:text-[13px] font-medium text-gray-500 whitespace-nowrap">
                <Icon className="h-3 w-3 sm:h-3.5 sm:w-3.5 shrink-0 text-[#1A7A52]" strokeWidth={1.5} />
                {label}
              </span>
            ))}
          </div>
