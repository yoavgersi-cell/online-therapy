    ...
    <>
        ...
        <div className="sticky top-6 space-y-5">
          {sortedBlocks.map((block, i) => {
                    ...
                    {selectedProviders.map((provider, pi) => (
                      <div key={provider.id} className="flex items-center gap-3">
                        ...
                        <ProviderCta
                          ...
                          className="shrink-0 rounded-lg bg-[#1A7A52] px-3 py-1.5 text-[12px] font-bold text-white transition-colors hover:bg-[#14634A]"
                        >
                          Visit
                        </ProviderCta>
                      </div>
            ...
            if (block.type === "quizCta") {
              return (
                <div key={i} className="rounded-xl border border-[#1A7A52]/15 bg-[#1A7A52]/[0.03] p-5">
                  ...
                  <Link
                    ...
                    className="mt-4 flex h-[40px] w-full items-center justify-center gap-1.5 rounded-lg bg-[#1A7A52] text-[13px] font-bold text-white transition-colors hover:bg-[#14634A]"
                  >
                ...
                <div key={i} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {selectedArticles.map((article) => (
                      ...
                      >
                        <p className="text-[13px] font-semibold leading-snug text-[#191919] group-hover:text-[#1A7A52] transition-colors">
                          {article.title}
                        </p>
                        {article.category && (
      ...
      <div className="lg:hidden mt-10 space-y-6">
        {sortedBlocks.map((block, i) => {
          if (block.type === "quizCta") {
            return (
              <div key={i} className="rounded-xl border border-[#1A7A52]/15 bg-[#1A7A52]/[0.03] p-5 text-center">
                ...
                <Link
                  href={config.quizCta.ctaUrl}
                  className="mt-4 inline-flex h-[44px] items-center justify-center gap-1.5 rounded-lg bg-[#1A7A52] px-6 text-[14px] font-bold text-white"
                >
                  {config.quizCta.ctaText}
                  <ArrowRight className="h-3.5 w-3.5" strokeWidth={2.5} />
                </Link>
                  ...
                  {selectedProviders.map((provider, pi) => (
                    <div key={provider.id} className="flex items-center gap-3">
                      ...
                      <ProviderCta
                        ...
                        pageType={pageType}
                        sourceFlow={sourceFlow}
                        className="shrink-0 rounded-lg bg-[#1A7A52] px-3 py-1.5 text-[12px] font-bold text-white"
                      >
                        Visit
                      </ProviderCta>
                    </div>
