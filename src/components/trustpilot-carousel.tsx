  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-gray-200 bg-white p-4">
      ...
      <div className="mt-auto flex items-center gap-2">
        <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#1A7A52]/10 text-[11px] font-bold text-[#1A7A52]">
          {r.name.charAt(0)}
        </div>
              ...
              {Array.from({ length: pageCount }).map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i === page ? "w-5 bg-[#1A7A52]" : "w-1.5 bg-gray-200 hover:bg-gray-300"
                  }`}
                />
              ))}
            </div>
          ...
          <div className="flex gap-1.5">
            {reviews.map((_, i) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i === current ? "w-5 bg-[#1A7A52]" : "w-1.5 bg-gray-200"
                }`}
              />
            ))}
          </div>
