...
export default function FindYourMatchPage() {
  ...
  useEffect(() => {
    fetch("/api/config", {
      headers: { Authorization: `Bearer ${sessionStorage.getItem("admin_token") || "edtreatment2026"}` },
    })
      .then((r) => r?.json())
    ...
    return (
      <>
        ...
        <div className="flex min-h-[60vh] items-center justify-center">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-gray-200 border-t-[#1A7A52]" />
        </div>
      </>
    );
  }
  ...
  if (phase === "welcome") {
    ...
    return (
      <>
      <HideChrome />
      <div className={`bg-[#F1F8F4] px-5 py-10 sm:py-20 transition-all duration-[280ms] ease-out ${phaseTransitioning ? "opacity-0 scale-[0.97]" : "opacity-100 scale-100"}`}>
        <div className="mx-auto w-full max-w-[540px] text-center">
          ...
          <button
            ...
            className="mt-8 inline-flex h-[46px] w-full max-w-[280px] items-center justify-center rounded-xl bg-[#1A7A52] text-[15px] font-bold text-white shadow-lg transition-all hover:bg-[#14634A] hover:shadow-xl sm:mt-10 sm:h-[50px] sm:w-auto sm:px-14 sm:text-[18px]"
          >
    ...
    return (
      <><HideChrome /><div className={`flex min-h-[60vh] flex-col items-center justify-center px-6 transition-all duration-[280ms] ease-out ${phaseTransitioning ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}>
        ...
        <div className="relative flex h-14 w-14 items-center justify-center">
          <div className="absolute inset-0 animate-ping rounded-full bg-[#1A7A52]/10" style={{ animationDuration: "1.5s" }} />
          <div className="absolute inset-0 animate-pulse rounded-full bg-[#1A7A52]/5" style={{ animationDuration: "2s" }} />
          <div className="relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-[#1A7A52] to-[#1a7ab5] shadow-lg">
        ...
        <div className="mt-3 flex items-center gap-1.5">
          {[0, 1, 2].map((i) => (
            <div
              key={i}
              className="h-1.5 w-1.5 rounded-full bg-[#1A7A52]/40 animate-bounce"
      ...
      <><HideChrome /><div className={`flex min-h-[60vh] flex-col items-center pt-20 sm:pt-28 px-6 bg-white transition-all duration-[280ms] ease-out ${phaseTransitioning ? "opacity-0 scale-95" : "opacity-100 scale-100"}`}>
        <div className="flex flex-col items-center">
          ...
          <p className="mb-1.5 text-[14px] font-medium text-gray-500 sm:text-[15px]">
            Comparing top online therapy platforms...
          </p>
          <p
            key={`text-${loadingIdx}`}
          ...
          <div className="mt-12 flex flex-col items-center gap-1">
            <p className="text-[22px] font-extrabold text-[#191919] sm:text-[26px]">12,000+</p>
            <p className="text-[12px] text-gray-400 sm:text-[13px]">people found their therapy platform with us</p>
          </div>
        </div>
      </div></>
    );
  ...
  if (phase === "results" && matchedProviders.length > 0) {
    ...
    return (
      <><HideChrome /><div className="bg-[#F1F8F4] py-8 sm:py-14 overflow-x-hidden animate-[fadeSlideUp_0.5s_ease-out]">
        <div className="mx-auto max-w-[800px] px-4 overflow-hidden">
          ...
          <div className="mb-3">
            <div className="mb-2 flex items-center gap-2 px-1">
              <span className="rounded-full bg-[#1A7A52] px-4 py-1 text-[12px] font-bold text-white sm:text-[13px]">
            ...
            <div className="mt-2 flex items-center gap-3 px-1">
              {featured.reasons.map((r, i) => (
                <span key={i} className="flex items-center gap-1 text-[11px] text-gray-500 sm:text-[12px]">
                  <Check className="h-3 w-3 shrink-0 text-[#1A7A52]" strokeWidth={2} />
                  {r}
                </span>
              ))}
            </div>
  ...
  return (
    <><HideChrome /><div className={`bg-[#F1F8F4] px-4 py-3 sm:py-16 transition-all duration-[280ms] ease-out ${phaseTransitioning ? "opacity-0 scale-[0.97]" : "opacity-100 scale-100"}`}>
      <div className="mx-auto max-w-[720px]">
        {/* Progress section */}
        <div className="mb-3 flex items-center gap-3 sm:mb-4">
          <div className="h-[3px] flex-1 rounded-full bg-gray-200">
            <div
              className="h-full rounded-full bg-[#1A7A52] transition-all duration-500 ease-out"
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center rounded-xl border-[1.5px] px-4 py-2.5 text-left text-[16px] font-medium transition-all duration-200 sm:px-5 sm:py-3 sm:text-[17px] active:scale-[0.99] ${
                        isSelected
                          ? "border-[#1A7A52] bg-[#1A7A52]/[0.04] text-[#191919] shadow-sm"
            ...
            {isDropdown && (
              <>
                ...
                <button
                  ...
                  className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#1A7A52] py-3.5 text-[16px] font-bold text-white transition-all hover:bg-[#14634A] disabled:opacity-40 sm:mt-5 sm:py-4 sm:text-[18px]"
                >
                  Get My Results
    ...
    <div className="mt-4 sm:mt-5">
      ...
      <div className="relative mb-3">
        ...
        <input
          ...
          className={`w-full rounded-xl border py-2.5 pl-10 pr-4 text-[14px] placeholder-gray-400 focus:border-[#1A7A52] focus:bg-white focus:outline-none sm:py-3 sm:text-[15px] ${
            selected && !search ? "border-[#1A7A52] bg-[#1A7A52]/5 text-[#1A7A52] font-medium" : "border-gray-200 bg-[#fafbfc] text-gray-800"
          }`}
        />
      </div>
          ...
          return (
            <button
              ...
              className={`flex w-full items-center gap-3 border-b border-gray-100 px-4 py-2.5 text-left text-[14px] font-medium transition-colors last:border-0 sm:px-5 sm:py-3 sm:text-[15px] ${
                isActive
                  ? "bg-[#1A7A52]/5 text-[#1A7A52]"
                  : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              <MapPin className={`h-3.5 w-3.5 shrink-0 ${isActive ? "text-[#1A7A52]" : "text-gray-300"}`} strokeWidth={1.5} />
              {opt.label}
            </button>
          );
        })}
